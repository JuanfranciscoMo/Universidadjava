...
public class ServicioSnacksArchivos implements IServicioSnacks {
    ...
    private List<Snack>obtenerSnacks(){
        ...
        try{
            List<String> lienas = Files.readAllLines(Paths.get(NOMBRE_ARCHIVO));
            for (String linea:lienas) {
                String[] lienaSnack = linea.split(","); // parseo separado por una coma
                var idSnack = lienaSnack[0]; // No se usa
                var nombre = lienaSnack[1];
                var precio = Double.parseDouble(lienaSnack[2]);
                var snack = new Snack(nombre, precio);
                snacks.add(snack); // agregamos el snack leidoa la lista
            }
        }catch (Exception e){
    ...
    public void agregarSnack(Snack snack) {
        ...
        // 1. Se guerda en la lista en memoria
    ...
    public void mostrarSnack() {
        ...
        for (var snack: this.snacks){
            invetarioSnack += snack.toString() +  "\n";
        }
        System.out.println(invetarioSnack);
    }
